			...
			switch(wybierz) {
				...
				case 3:
					...
					if(iter.next().getIndex().contains(wybor)) {   //dziala 
						// jesli zadziala to ma pokazac dane dodanego produktu	// DZIALA
						System.out.println(produkt.get(wyborInt).index);
						System.out.println(produkt.get(wyborInt).nazwa);
						System.out.println(produkt.get(wyborInt).ilosc);
						System.out.println(produkt.get(wyborInt).cena);
						// teraz stworzyc obiekkt klasy Koszyk i wrzucic tam te dane
						koszyk.add(obiektKoszyk.dodajDoKoszyka(produkt.get(wyborInt).index, produkt.get(wyborInt).nazwa, produkt.get(wyborInt).ilosc, produkt.get(wyborInt).cena));
					} else {
						System.out.println("");
					 }
					}
				...
				case 4: 
					...
					System.out.println("Do zapłaty hurtowy: "+hurtowy+" zł");
					break;
				case 5:
					System.exit(0);
			}
