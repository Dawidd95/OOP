...
class Utwor {
	...
	String zmienTytul(String nowy) {
		this.tytul_utworu = nowy;
		return this.tytul_utworu;
	}
	
	String zmienWykonawce(String nowy) {
		this.autor_utworu = nowy;
		return this.autor_utworu;
	}
}
		...
		do {
			...
			switch(wybierz) {
				...
				case 2: 
					...
					System.out.println("Wprowadz tytul utworu BEZ SPACJI: ");

					String tytul = scanner.next();
							
					System.out.println("Wprowadz autora utworu BEZ SPACJI: ");
				...
				case 4: 
					...
					for(int i = 0; i<utwor.size(); i++) {
						...
						if(utwor.get(i).getId().contains(id2)) {
							numberTrue3++;
						} 
					}
					...
					System.out.println("Wprowadz nowy tytul BEZ SPACJI");
					
					String nowyTytul = scanner.next();
					
					System.out.println("Wprowadz nowego wykonawce BEZ SPACJI");
					...
					for(int i = 0; i<utwor.size(); i++) {				
						if(utwor.get(i).getId().contains(id2)) {  
							...
							//
							utwor.get(i).zmienTytul(nowyTytul);
							utwor.get(i).zmienWykonawce(nowyWykonawca);
						}
					}
